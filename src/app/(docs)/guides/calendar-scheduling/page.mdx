...
When a customer starts from WhatsApp, the booking link uses the canonical Chatty host (`chatty.personaliai.com`) and carries a short-lived, phone-bound signature. After the visitor confirms a slot, the calendar invite and meeting details can be sent back into the same WhatsApp thread.
